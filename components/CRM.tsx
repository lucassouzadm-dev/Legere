...
import { exportClientsPDF, exportClientsExcel } from '../services/exportService';
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            onClick={async () => {
              ...
              try { await exportClientsPDF(clients, getCurrentTenant()?.name ?? 'Legere'); }
              finally { setExportingPdf(false); }
            }}
            disabled={exportingPdf || clients.length === 0}
          ...
          <button
            onClick={async () => {
              ...
              try { await exportClientsExcel(clients, getCurrentTenant()?.name ?? 'Legere'); }
              finally { setExportingXls(false); }
            }}
            disabled={exportingXls || clients.length === 0}
            className="bg-white dark:bg-slate-800 border dark:border-slate-700 text-green-700 dark:text-green-400 px-4 py-2 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-gray-50 dark:hover:bg-slate-700 transition-all disabled:opacity-50"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
            {exportingXls ? 'Gerando...' : 'Excel'}
          </button>
