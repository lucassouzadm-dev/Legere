...
import { exportCasesPDF } from '../services/exportService';
...
const Cases: React.FC<CasesProps> = ({ cases, setCases, clients, users }) => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      await exportCasesPDF(filteredCasesList, users, tenantName);
    } catch (error) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            disabled={exporting || filteredCasesList.length === 0}
            className="bg-gold-800 text-white px-6 py-2 rounded-xl text-sm font-bold shadow-lg hover:bg-navy-800 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
          >
